...
const getClientsModule = (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    Controller.getClientsModule(Number(req.query.idUser), Number(req.query.permissionId))
        .then((data) => {
            success({ req, res, message: data });
        })
        .catch(next)
...
router
    ...
    .get("/clients", secure(undefined, undefined, EModules.users, 1), getClientsModule)
