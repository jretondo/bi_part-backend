...
export = () => {
    ...
    const getClientsModule = async (userId: number, permissionId: number) => {
        //Find a clients list with the basic permissions for a specific module
        return AdminPermission.findAll({ where: [{ user_id: userId }, { permission_id: permissionId }, { client_enabled: true }, { permission_grade: { [Op.gt]: 0 } }], include: Client })
    }
